...
export function PurchaseButton({
  ...
  async function handleTogglePurchase() {
    ...
    // Set processing first, before updating claimed state
  ...
  return (
    <button
      ...
      className={`w-full px-4 py-2 rounded-lg font-medium text-sm transition-all duration-200 disabled:cursor-not-allowed disabled:opacity-50 ${
        claimed
    ...
    >
      {isProcessing ? (
        <span className="flex items-center justify-center gap-2">
          ...
          {!claimed ? 'Claiming...' : 'Unclaiming...'}
        </span>
      ) : (
        <>{claimed ? "✓ I'm getting this" : 'Claim Item'}</>
      )}
    </button>
  )
}
